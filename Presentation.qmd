...
format:
  revealjs:
    ...
    theme: white
    logo: "Data/resources/LMU_Logo.svg"
editor: visual
---
...
#| output: false
source("source_all.R")
DataPrep(method = "save", colors = FALSE)
```
...
Q1_plots <- Q1(readRDS("Data/cleaned/Worldbank.RDS"), method = "return")
...
-   Sensitivätsanalyse bezüglich des Extremwerts Aruba verdeutlicht dies
...
-   Entfernen des Extremwerts Aruba ändert daran nichts (Sensitivitätsanalyse)
...
Q2_plots <- Q2(readRDS("Data/cleaned/Worldbank.RDS"), method = "return")
Q2_plots$missingness_1
```
...
-   Auch für die Länder mit den durchschnittlich höchsten Bildungsquoten
...
Q3_plots <- Q3(readRDS("Data/cleaned/Worldbank.RDS"), method = "return")
Q3_plots$missingness
```
...
Q4_plots <- Q4(readRDS("Data/cleaned/Worldbank.RDS"), method = "return")
Q4_plots$missingness
```
...
-   Für die Gesamtheit der ärmeren Volkswirtschaften[^1] verschwindet der Trend
-   Auf Länderebene existiert der Trend weiter

[^1]: gemäß Klassifikation durch die Weltbank von 2023
...
Bei den reicheren Volkswirtschaften[^2] existiert der Trend sowohl übergreifend, also auch auf Ebene der Nationen.

[^2]: gemäß Klassifikation durch die Weltbank von 2023
...
Q5_plots <- Q5(readRDS("Data/cleaned/Worldbank.RDS"), method = "return")
Q5_plots$plot1
```
...
In den vorliegenden Daten liegt kein Zusammenhang zwischen Landesgröße und Korrelationskoeffizient[^3] vor.

[^3]: bzgl. CO2 Emissionen pro Kopf und landwirtschaftlicher Nutzfläche
...
::: incremental
-   Zugang zu Elektrizität korreliert im Datensatz **stark positiv** mit dem Nettonationaleinkommen pro Kopf
...
spearman_examples(method = "return")
```

## Volkswirtschaftliche Gesamtrechnung

```{r graph VGR}
graph_VGR(method = "save")
```
...
aggr_examples(readRDS("Data/cleaned/Worldbank.RDS"), method = "return")
```

## Klassifizierung durch Weltbank
...
```{r Klassifizierung, fig.align='center'}
country_classification(data = readRDS("Data/cleaned/Worldbank.RDS"), method = "return")
```
